"""Manual portfolio position import – works in any environment (web, local, etc.).

Users paste or type their holdings directly; no broker client or local SDK required.
Positions are stored in the same ``ImportedPortfolioPositionDB`` table used by the
QMT importer, with ``source = "manual"``.
"""
...
def sync_manual_portfolio(
    db: Session,
    user_id: str,
    positions: list[dict[str, Any]],
    auto_apply_scheduled: bool = True,
) -> dict[str, Any]:
    """Replace the manual-imported positions snapshot with *positions*.
...
def _normalize_code(value: Any) -> str | None:
    ...
    if not text:
        return None
    # Accept "600519.SH" directly
    if _CODE_RE.match(text):
        return text
    # Accept bare 6-digit codes and infer exchange
