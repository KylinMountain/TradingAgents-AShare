...
<!-- VERDICT: {{"direction": "看多", "reason": "核心结论"}} -->""",
    ...
    2. **计算具体仓位**：根据 `final_trade_decision` 的信心（Confidence）和风险（Max Loss），使用凯利公式或固定比例法建议具体买入/卖出股数或金额。
    3. **行业对冲检查**：如果该标的所属行业在用户当前持仓中占比过高（>30%），必须明确给出减速或对冲建议。
    4. **生成操作指令**：输出明确的指令，例如“买入 100 股 [代码] @ [入场价]”，“将该股仓位从 10% 提升至 15%”等。
