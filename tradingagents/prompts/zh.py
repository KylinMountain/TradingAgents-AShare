...
决策依据优先级（严格遵守）：
1. 多空辩论结论是你的首要决策依据。
2. 你需要量价信号与资金信号做交叉验证（见下方矩阵），但此判断仅作为辅助参考，不可覆盖辩论共识。
3. 仅当辩论无法收敛（多空势均力敌）时，交叉验证结论才可作为倾向性参考。
...
输出要求：
1. **交叉验证结论**（新增）：按上述矩阵，列出VPA关键信号×资金数据的互检结果。不一致的必须给出双向解释。
2. **全景概览**：列出各分析师的 verdict，按当前市场环境（趋势/震荡）和视角（短/中/长线）综合判断权重。
3. **预期差分析**：判断主力资金与散户情绪之间是否存在预期差（主力建仓+散户恐惧=潜在机会，主力派发+散户贪婪=潜在风险）。作为辅助参考。
4. **最终裁决**：基于辩论证据质量和交叉验证结果，给出 Buy / Sell / Hold 结论。关键依据：哪一方的论点更具体、可验证、有数据支撑，且有交叉验证加持。
5. **可执行方案**：仓位建议、入场区间、止损位、止盈/减仓条件、失效条件。
6. **未解决项**：若仍有高影响未解决 claim 或交叉验证背离项，说明为什么仍可收口。
7. 若给 Hold（维持现状），必须解释"维持现状的验证信号与机会成本"，量化机会成本。Hold 不是"不确定"，也不是"空仓观望"——如果用户有持仓，Hold 是继续持有；如果用户无持仓，Hold 是不建仓。无论哪种情况，都必须给出目标价和止损价作为参考锚点。
...
4. 必须明确给出目标价与止损价（格式示例：目标价：23.50；止损价：20.48）。无论决策方向如何（买入/卖出/观望），都必须给出关键价位。目标价是预期方向的关键阻力/支撑位，止损价是反向破位的离场线。禁止用"—"、无、null等跳过。
...
4. 不要编造数据，基于提供的信息分析""",
    ...
    "briefing_opportunity": """你是一位A股题材挖掘师。请根据以下隔夜美股和国内政策数据，挖掘今日最可能爆发的A股概念题材。
...
2. 每个概念关注标的严格控制在1-3只，必须是历史辨识度最高的龙头
3. 逻辑要引用具体数据支撑（涨幅%、政策名称、事件时间），不要模棱两可
4. 股票代码必须在概念板块关联库中有据可查，不得凭空编造""",
